...
structure CoreIntInf :> sig
   ...
   *)
    datatype rep = BI of { negative : bool, digits : word63 list }
   ...
   *)
    val makeNegInf : word63 list -> intinf
    val makePosInf : word63 list -> intinf

  (* In the common case where only one big digit is involved, use
   * a shortcut without list allocation:
   *)
    val makeSmallNegInf : word63 -> intinf
    val makeSmallPosInf : word63 -> intinf

  (* For ~base63 < i < base63 we have lowValue i = i.
   * For other i we have lowValue i = ~base63 (= neg_base_as_int).
  ...
  (* add one to a list of digits *)
    val natinc : word63 list -> word63 list
  ...
  (* support for scanning and formatting: *)
    val natdivmodd : word63 list * word63 -> word63 list * word63
    val natmadd : word63 * word63 list * word63 -> word63 list
    ...
    val wToI64 : word63 -> int64  = InLine.copy_word63_to_int64
    val w64ToI64 : word64 -> int64 = InLine.copy_word64_to_int64
    val wToW64 : word63 -> word64 = InLine.unsigned_word63_to_word64
    val i64ToW : int64 -> word63 = InLine.trunc_int64_to_word63
    val i64ToW64 : int64 -> word64 = InLine.copy_int64_to_word64
    val w64ToW : word64 -> word63 = InLine.word64_to_word63
    val ~ : int -> int = InLine.int64_neg
    infix || && >> ^ <<
    ...
    val hBaseBits64 : word64 = 0w31 (* half number of bits per digit *)
    val baseBits63 : word63 = InLine.word63_lshift (0w31, 0w1)
    val baseBits64 : word64 = InLine.unsigned_word63_to_word64 baseBits63
    val baseBits : word64 = baseBits64

    val base63 : word63 = InLine.word63_lshift (0w1, baseBits64)
    val base64 = wToW64 base63
    val base = base64
    val maxDigit63 : word63 = InLine.word63_sub(base63, 0w1)
    val maxDigit64 = wToW64 maxDigit63
    val maxDigit = maxDigit64
    val maxDigitL : word63 = 0wx7fffffff	(* lower half of maxDigit *)
    val maxDigitL64 = wToW64 maxDigitL
    val neg_base_as_int : int = ~0x4000000000000000

    val gap : word63 = InLine.word63_sub (0w64, baseBits63) (* 64 - baseBits *)
    val slc : word63 = InLine.word63_sub (baseBits63, gap)  (* baseBits - gap *)
  ...
  (* truncate intinf to word64 *)
    fun truncInf64 i = let
	  ...
	  val b = (case digits
		 of [] => 0w0
		  | [d] => wToW64 d
		  | d0 :: d1 :: _ => wToW64 d0 || (wToW64 d1 << baseBits64)
		(* end case *))
	  in
  ...
  (* sign-extend an int64 to an intinf *)
    fun extend64Inf i64 = let
	  fun e (negative, w63) = BI{
		    negative = negative,
		    digits = if InLine.word63_ge (w63, base63)
			then [InLine.word63_sub (w63, base63), 0w1]
			else [w63]
		  }
  ...
  (* zero-extend a word64 to an intinf *)
    fun copy64Inf w64 = let
	  val digits = if InLine.word64_eql (w64, 0w0)
		  ...
		  then [w64ToW (w64 && maxDigit64), w64ToW (w64 >> baseBits64)]
		  else [w64ToW w64]
	  in
    ...
    fun lowValue i = (case concrete i
	   of BI { digits = [], ... } => 0
		...
		InLine.copy_word63_to_int64 d
	    | BI { digits = [d], negative = true } =>
		InLine.int64_neg (InLine.copy_word63_to_int64 d)
	    | _ => neg_base_as_int
	  (* end case *))
    ...
    fun adddig (d1, d2) : {carry: bool, res: word63} =
        let val sum = InLine.word63_add (d1, d2)
        in
	    {carry = InLine.word63_ge (sum, base63),
	     res = InLine.word63_andb (sum, maxDigit63) }
        end

    (* add one to nat *)
    fun natinc [] = [0w1]
      | natinc (x :: xs) =
	  if InLine.word63_eql (x, maxDigit63) then 0w0 :: natinc xs
	  else InLine.word63_add (x, 0w1) :: xs

    fun natdec (0w0 :: xs) = maxDigit63 :: natdec xs
      | natdec [0w1] = []
    ...
    fun natadd1 (x, []) = natinc x
      ...
      | natadd1 (x :: xs, y :: ys) = let
	    ...
	    val (carry, res) =
		if InLine.word63_eql (res, maxDigit63) then (true, 0w0)
    ...
    fun natsub (xs, [], false) = xs
      ...
      | natsub (x :: xs, y :: ys, c) = let
		...
		if InLine.word63_lt (x, y') then
		    (InLine.word63_sub (InLine.word63_add (x, base63), y'), true)
    ...
    fun ddmul (x, y) = let
	  ...
	  val (bh, bl) = hl b
	  val l0 = InLine.word64_add (c, InLine.word64_lshift (bl, hBaseBits64))
	  val l64 = l0 && maxDigit64
	  val lc = l0 >> baseBits64
    ...
    fun consd (0w0: word63, []) = []
      | consd (x, xs) = x :: xs

    fun scale w = InLine.word63_div (base63, InLine.word63_add (w, 0w1))

  (* returns length-1 and last element *)
    fun length'n'last [] = (0, 0w0)	(* should not happen *)
      | length'n'last [x: word63] = (0, x)
      | length'n'last (_ :: l) = let
	  val (len, last) = length'n'last l
	  in
	    (InLine.int64_add (len, 1), last)
	  end

    fun nth (_, []: word63 list) = 0w0
      | nth (0, x :: _) = x
      | nth (n, _ :: xs) = nth (InLine.int64_sub (n, 1), xs)

    (* divide DP number by digit; assumes u < i , i >= base63/2 *)
    fun natdivmod2 ((u,v), i) =
	let fun low w = InLine.word63_andb (w, maxDigitL)
	    fun high w = InLine.word63_rshiftl (w, hBaseBits64)
	    val (vh, vl) = (high v, low v)
	    val (ih, il) = (high i, low i)

	    fun adj (q, r, vx) =
		let val x = InLine.word63_add (InLine.word63_lshift (r, hBaseBits64), vx)
	...
	in (InLine.word63_add (InLine.word63_lshift (q1, hBaseBits64), q0), r0)
	end
            ...
            fun divl [] = ([], [])
              | divl (d::r) = let
                    ...
                    fun msds ([],_) = (0w0,0w0)
                      | msds ([d],0) = (0w0,d)
                      | msds ([d2,d1],0) = (d1,d2)
                      | msds (d::r,i) = msds (r,InLine.int64_sub (i, 1))
                    val (m1,m2) = msds (m, ln)
                    val tq = if InLine.= (m1, n1) then maxDigit63
  ...
  (* For mod and rem we special-case a divisor of 2 (common even-odd test) *)
    ...
    fun natpow (_, 0) = [0w1]
      | natpow ([], n) = if InLine.int64_lt (n, 0) then raise Assembly.Div else []
      | natpow (x, n) =
	  if InLine.int64_lt (n, 0) then []
	       ...
	       in
		   exp (x, InLine.trunc_int64_to_word63 n) (* DEFAULT64: trunc? *)
	       end
	...
	in
	    abstract (bi { negative = negative andalso
	                              InLine.int64_eql (InLine.int64_rem (n, 2), 1),
