...
structure CoreIntInf :> sig
  ...
  (* support for scanning and formatting: *)
    ...
    val wToI64 : word -> int64  = InLine.copy_word64_to_int64
    val w64ToI64 : word64 -> int64 = InLine.copy_word64_to_int64
    val wToW64 : word -> word64 = (fn x => x)
    val i64ToW : int64 -> word = InLine.copy_int64_to_word64
    val i64ToW64 : int64 -> word64 = InLine.copy_int64_to_word64
    val w64ToW : word64 -> word = (fn x => x)
    ...
    val baseBits : word = InLine.word64_lshift (hBaseBits, 0w1)
    val base : word = InLine.word64_lshift (0w1, baseBits)
    val base64 = wToW64 base
    val maxDigit : word = InLine.word64_sub(base, 0w1)
    ...
    val gap : word = InLine.word64_sub (0w64, baseBits) (* 64 - baseBits *)
    val slc : word = InLine.word64_sub (baseBits, gap)  (* baseBits - gap *)
  ...
  (* sign-extend an int64 to an intinf *)
    fun extend64Inf i64 = let
	  fun e (negative, w63) = BI{
		    negative = negative,
		    digits = if InLine.word64_ge (w63, base)
			then [InLine.word64_sub (w63, base), 0w1]
			else [w63]
		  }
  ...
  (* zero-extend a word64 to an intinf *)
    ...
    fun lowValue i = (case concrete i
	   of BI { digits = [], ... } => 0
		...
		InLine.copy_word64_to_int64 d
	    | BI { digits = [d], negative = true } =>
		InLine.int64_neg (InLine.copy_word64_to_int64 d)
	    | _ => neg_base_as_int
	  (* end case *))
    ...
    fun dcmp (x, y) =
	if InLine.word64_lt (x, y) then LESS else
	if InLine.word64_gt (x, y) then GREATER else
	EQUAL
    ...
    fun adddig (d1, d2) : {carry: bool, res: word} =
        let val sum = InLine.word64_add (d1, d2)
        in
	    {carry = InLine.word64_ge (sum, base),
	     res = InLine.word64_andb (sum, maxDigit) }
        end

    (* add one to nat *)
    fun natinc [] = [0w1]
      | natinc (x :: xs) =
	  if InLine.word64_eql (x, maxDigit) then 0w0 :: natinc xs
	  else InLine.word64_add (x, 0w1) :: xs

    fun natdec (0w0 :: xs) = maxDigit :: natdec xs
      | natdec [0w1] = []
      | natdec (x :: xs) = InLine.word64_sub (x, 0w1) :: xs
    ...
    fun natadd1 (x, []) = natinc x
      ...
      | natadd1 (x :: xs, y :: ys) = let
	    ...
	    val (carry, res) =
		if InLine.word64_eql (res, maxDigit) then (true, 0w0)
		else (carry, InLine.word64_add (res, 0w1))
	in
    ...
    fun natsub (xs, [], false) = xs
      ...
      | natsub (x :: xs, y :: ys, c) = let
	    val y' = if c then InLine.word64_add (y, 0w1) else y
	    val (res, carry) =
		if InLine.word64_lt (x, y') then
		    (InLine.word64_sub (InLine.word64_add (x, base), y'), true)
		else (InLine.word64_sub (x, y'), false)
	in
	    case natsub (xs, ys, carry) of
		[] => if InLine.word64_eql (res, 0w0) then [] else [res]
	      | more => res :: more
	end
    ...
    fun natmadd (0w0, _, 0w0) = []
	  ...
	  val h' = if carry then InLine.word64_add (h, 0w1) else h
	  in
	    l' :: natmadd (w, xs, h')
	  end
    ...
    fun scale w = InLine.word64_div (base, InLine.word64_add (w, 0w1))
  ...
  (* returns length-1 and last element *)
	...
	let fun low w = InLine.word64_andb (w, maxDigitL)
	    fun high w = InLine.word64_rshiftl (w, hBaseBits)
	    val (vh, vl) = (high v, low v)
	    val (ih, il) = (high i, low i)

	    fun adj (q, r, vx) =
		let val x = InLine.word64_add (InLine.word64_lshift (r, hBaseBits), vx)
		    val y = InLine.word64_mul (q, il)
		    fun loop (q, x) =
			if InLine.word64_ge (x, y) then (q, InLine.word64_sub (x, y))
			else loop (InLine.word64_sub (q, 0w1), InLine.word64_add (x, i))
		in loop (q, x)
		end

	    val q1 = InLine.word64_div (u, ih)
	    val r1 = InLine.word64_mod (u, ih)
	    val (q1, r1) = adj (q1, r1, vh)
	    val q0 = InLine.word64_div (r1, ih)
	    val r0 = InLine.word64_mod (r1, ih)
	    val (q0, r0) = adj (q0, r0, vl)
	in (InLine.word64_add (InLine.word64_lshift (q1, hBaseBits), q0), r0)
	end
    ...
    fun natdivmodd (m, 0w1) = (m, 0w0) (* speedup *)
      | natdivmodd (m, i) = let
            val scale = scale i
            val i' = InLine.word64_mul (i, scale)
    ...
    fun natdivmod (m, []) = raise Assembly.Div
      ...
      | natdivmod (m, [d]) = let
        ...
        in (qt, if InLine.word64_eql (rm, 0w0) then [] else [rm]) end
      | natdivmod (m, n) = let
	    ...
	    val scale = scale last
            ...
            fun divl [] = ([], [])
              | divl (d::r) = let
                	...
                	handle Negative => try (InLine.word64_sub (q,0w1),
						natsub0 (qn', n'))
  ...
  (* For mod and rem we special-case a divisor of 2 (common even-odd test) *)
    fun mod' (_, BI { digits = [], ... }) = raise Assembly.Div
	  ...
	  if InLine.word64_eql (InLine.word64_andb (low, 0w1), 0w0) then zero
    ...
    fun natpow (_, 0) = [0w1]
	  ...
	  if InLine.int64_lt (n, 0) then []
		     ...
		     | exp (m, 0w1) = m
		     | exp (m, n) = let
			   val x = exp (m, InLine.word64_rshiftl (n, 0w1))
			   val y = natmul (x, x)
		       in
			   if InLine.word64_eql (InLine.word64_andb (n, 0w1), 0w0)
                           then y
			   else natmul (y, m)
		       end
	       in
		   exp (x, InLine.copy_int64_to_word64 n)
	       end
