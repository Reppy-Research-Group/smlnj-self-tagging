...
structure Core =
   ...
   *)
    ...
    local val ieql : int * int -> bool = InLine.int64_eql
          val peql : 'a * 'a -> bool = InLine.ptr_eql
          val ineq : int * int -> bool = InLine.int64_neq
	  val i64eq : int64 * int64 -> bool = InLine.int64_eql
          val boxed : 'a -> bool = InLine.boxed
          val op + : int * int -> int = InLine.int64_add
          val op - : int * int -> int = InLine.int64_sub
          val op * : int * int -> int = InLine.int64_mul
	  ...
	  val recSub : ('a * int) -> 'b = InLine.recordSub
          ...
          val andb : int * int -> int = InLine.int64_andb
    ...
    in
      ...
      *)
       val max_length = let
	    val op - = InLine.word64_sub
	    infix << val op << = InLine.word64_lshift
	    val int = InLine.copy_word64_to_int64
	    in
	      int ((0w1 << (0w63 - width_tags)) - 0w1) (* DEFAULT64: ? *)
	    end
       ...
       *)
	   ...
	   fun force (x : 'a susp) =
	       if InLine.int64_eql((InLine.getspecial x),TSUS)
