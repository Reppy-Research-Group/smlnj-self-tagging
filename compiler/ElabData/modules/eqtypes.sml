	...
	raise Subscript))
	 ...
	 (case (!eq, kind) of
	      ...
	      let val _ = debugmsg (">>checkTyc: " ^
				    Symbol.name (IP.last (path, SS.errorTycId)) ^ " " ^
				    Int.toString index)
		  ...
		  case !eq
		   of (YES | NO | IND) => ()
		    | DATA =>
		      bug ("checkTyc[=>DATA]: " ^ Symbol.name(IP.last (path, SS.errorTycId)))
		    | _ =>
		      bug ("checkTyc[=>other]: "^Symbol.name(IP.last (path, SS.errorTycId)))
	      end
	    | _ => ())
       | checkTyc _ = ()
in
