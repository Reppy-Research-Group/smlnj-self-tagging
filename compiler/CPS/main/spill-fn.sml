...
functor SpillFn (MachSpec : MACH_SPEC) : SPILL =
  struct
    ...
    fun pr msg = Control.Print.say(concat msg)
    ...
    (* Pretty printing *)
    fun dump(title, cpsFun) = if !debug_cps_spill
          then (
  ...
  fun markFpAndRec cpsFun = let
      ...
      fun recUses [] = ()
        | recUses ((CPS.VAR v, _) :: uses) = (case findrec v
           of NONE => recUses uses   (* not a record address *)
            | SOME n => (markrec(v, n+1); recUses uses)
          (* end case *))
        | recUses (_ :: uses) = recUses uses
      ...
      val () = if DEBUG
            then H.appi
              (fn (v, n) => if n >= 2
                  then pr[LV.lvarName v," uses=",i2s n,"\n"]
                  else ())
              recordSet
            else ()
    ...
    fun needsSpilling (TINFO{maxLive, isVar, ...}) cpsFun = let
        ...
        fun check S = let
            val n = card S
            in
              if n > !bandwidth then bandwidth := n else ();
         ...
         *)
        fun freevars e = (case e
             of CPS.APP(v,args)          => uses(v::args,O)
              ...
              | CPS.RCC(_,_,_,vl,wtl,e)  =>
                  uses(vl, foldl (fn((w,_),s) => def(w,s)) (freevars e) wtl)
        ...
        in
          { needsSpilling = needsSpilling, bandwidth = !bandwidth }
        end (* needsSpilling *)
     ...
     *     native integers, tagged ints, and pointers together.
    ...
    fun linearScan (TINFO{isFlt, maxLive, isVar}) cpsFun = let
        ...
        local
            ...
            fun gather(e, b, n) = let
                ...
                in
                  case e
                   of CPS.APP(v,args)        => uses(v::args, n)
                    ...
                    | CPS.RCC(_,_,_,vl,wtl,e) => let
                        val b = b+1
                        in
                          uses (vl, n);
                          List.app (fn (w, t) => def (w, t, b, n)) wtl;
                          gather (e, b, n+1)
                        end
        ...
        val () = if !debug_cps_spill
              then pr["CPS Spill: gather done\n"]
              else ()
        ...
        (* spillTable : lvar -> spillRecord * spill offset * cty *)
        exception SpillTable
        val spillTable : (CPS.value * int * CPS.cty) H.hash_table =
              H.mkTable(32, SpillTable)
         ...
         *)
        fun genSpillRec () = (case !currentSpillRecord
             of SOME x => x
              | NONE => let
                  val v = LV.namedLvar (Symbol.varSymbol "spillrec")
                  val e = CPS.VAR v
                  in
                    currentSpillRecord := SOME(v,e); (v, e)
                  end
            (* end case *))
        (*
        ...
        fun splitRecordConstruction (rk, vl, w) = let
            ...
            val n = length vars
            val _ = if n = 0
                  then error "CPS Spill: splitting constant record"
                  else ()
            val _ = if !debug_cps_spill_info
                  then pr["Splitting record ",LV.lvarName w," len=",i2s n,"\n"]
                  else ()
            val len = length vl
            val numVars = ref n
        ...
        fun scan (e, b, spOff) = let
            ...
            fun genSpills (L, spOff) = let
                val toSpills = card L - maxLive
                in
                  if toSpills > 0
                    then findGoodSpills(toSpills, L, spOff)
                    else (L, spOff)
                end
            ...
            fun scanList es = let
                ...
                fun f [] = (O, 0)
                  | f [e] = scan(e, b, spOff)
                  | f(e::es) = let
                    val (L1, spOff1) = scan(e, b, spOff)
                    val (L2, spOff2) = f es
                    in
                      (L1 \/ L2, Int.max(spOff1, spOff2))
                    end
                in
                  f es
                end (* scanList *)
            ...
            fun scanStmt (vs, es) = let
                val (L,spOff)  = scanList es         (* do continuation *)
                val L          = addUses(vs, L)      (* add uses *)
            ...
            val (L, numSpills) = (case e
                 of CPS.APP(v,args)        => scanStmt(v::args, [])
        ...
        val () = if !debug_cps_spill
              then pr["CPS Spill: scan done. Spilling ",i2s numSpills,"\n"]
              else ()
        (*
         ...
         *)
        fun emitPathReloads vl = let
            fun f ([], vl') = rev vl'
              | f ((v as CPS.VAR x, p)::vl, vl') = (case findSpill x
                 of NONE => f (vl, (v, p)::vl')
                  | SOME(spillRec, off, cty) => f (vl, (spillRec,CPS.SELp(off,p))::vl')
                (* end case *))
              | f (v::vl, vl') = f(vl, v::vl')
            in
              f (vl, [])
        ...
        fun proj (v, CPS.OFFp 0, e) = e v
          | proj (v, CPS.SELp(i,p), e) = let
            val v' = LV.mkLvar()
            val e  = e v'
            in
              CPS.SELECT(i, CPS.VAR v, v', U.BOGt, e)
            end
...
(* FIXME: note that `rk` can be RK_RECORD or RK_CONT, which is kind of bogus. *)
        ...
        fun assignToSplitRecord (v, e) = (case findSplitRecordArg v
             of SOME inits => let
                fun gen (SPLIT_RECORD_ITEM{
                    ...
                    val e = initRecordItem(record, kind, offset, v, path, e)
                    val n = !numVars - 1
                    in
                      numVars := n;
                      if n = 0
                        then createRecord(record, kind, len, consts, e)
                        else e
                    end
                in
                  List.foldr gen e inits
                end
              | NONE => e
            (* end case *))
         ...
         *-----------------------------------------------------------------*)
        fun rebuild e = let
            fun rewriteStmt (vs, es, f) = let
                val es = List.map rebuild es
                val (vs, g) = emitReloads vs
                in
                  g (f (vs, es))
                end
            fun rewrite (vs, w, e, f) = let
                val e = rebuild e
                val e = emitSpill(w, e)
               ...
               of CPS.APP(v,args) =>
                ...
                | CPS.RCC(k,l,p,vl,wtl,e) =>
                  rewrite' (vl, map #1 wtl, e,
                    fn (vl, wl, e) =>
                      CPS.RCC (k, l, p, vl, ListPair.map (fn (w, (_, t)) => (w, t)) (wl, wtl), e))
                | CPS.BRANCH(p,vl,c,x,y) =>
                  rewriteStmt (vl,[x,y], s'2 (fn (vl,x,y) => CPS.BRANCH(p,vl,c,x,y)))
